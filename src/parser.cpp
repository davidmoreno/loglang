...
AST Parser::parse_expr()
{
	...
	if (op.type==Token::OP && op.token=="="){
		AST op2=parse_expr();
		{
			...
			if (var!=nullptr)
				return std::make_unique<ast::Assign>(std::move(var->var), std::move(op2));
		}
		{
			ASTBase *op1p=&*op1;
			auto var=dynamic_cast<ast::Value_lvar*>(op1p);
			if (var!=nullptr)
				return std::make_unique<ast::LVAssign>(std::move(op1), std::move(op2));
		}
...
AST Parser::parse_term()
{
	...
	if (tok.type==Token::OPEN_BRACKET){
		auto expr=parse_expr();
		assert_next(Token::CLOSE_BRACKET);
		return std::make_unique<ast::Value_lvar>(std::move(expr));
	}
